...
export default function ProgressUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = progressRecord
      ? { ...initialValues, ...progressRecord, studentID, Step }
      : initialValues;
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : progressModelProp;
      const studentIDRecord = record ? record.studentID : undefined;
      const studentRecord = studentIDRecord
        ? (
            await client.graphql({
              query: getStudent.replaceAll("__typename", ""),
              variables: { id: studentIDRecord },
            })
          )?.data?.getStudent
        : undefined;
      setStudentID(studentIDRecord);
      setSelectedStudentIDRecords([studentRecord]);
      const StepRecord = record ? await record.Step : undefined;
      setStep(StepRecord);
      setProgressRecord(record);
    };
    queryData();
  }, [idProp, progressModelProp]);
  React.useEffect(resetStateValues, [progressRecord, studentID, Step]);
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            progressStepId: modelFields?.Step?.id ?? null,
          };
          await client.graphql({
