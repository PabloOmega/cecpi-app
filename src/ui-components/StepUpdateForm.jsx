...
import { getStep, getUnit, listUnits, listVideos } from "../graphql/queries";
import { updateStep } from "../graphql/mutations";
...
export default function StepUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = stepRecord
      ? { ...initialValues, ...stepRecord, Video, unitID }
      : initialValues;
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getStep.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getStep
        : stepModelProp;
      const VideoRecord = record ? await record.Video : undefined;
      setVideo(VideoRecord);
      const unitIDRecord = record ? record.unitID : undefined;
      const unitRecord = unitIDRecord
        ? (
            await client.graphql({
              query: getUnit.replaceAll("__typename", ""),
              variables: { id: unitIDRecord },
            })
          )?.data?.getUnit
        : undefined;
      setUnitID(unitIDRecord);
      setSelectedUnitIDRecords([unitRecord]);
      setStepRecord(record);
    };
    queryData();
  }, [idProp, stepModelProp]);
  React.useEffect(resetStateValues, [stepRecord, Video, unitID]);
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            stepVideoId: modelFields?.Video?.id ?? null,
            unitID: modelFields.unitID ?? null,
          };
          await client.graphql({
